...
const DrawflowEditor = () => {
    ...
    const checkNodeParams = () => {
        ...
        for (const node_id in usefulData) {
            if (
                usefulData[node_id].name == "e_store" ||
    ...
    const drawflowAPI = {
        ...
        setLockedMode: (locked) =>
            editorRef.current &&
            (editorRef.current.editor_mode = locked ? "fixed" : "edit"),
        ...
        setNodeTitle: (id, symbol, label) => {
            ...
            if (!editor) return;
            const titleEl = editor.precanvas.querySelector(
                `#node-${id} .title-box`
            );
            if (titleEl)
                titleEl.innerHTML = `<span class="node-symbol">${symbol}</span> ${label}`;
        },
    };
    ...
    `;
        ...
        const InitialValueField = `<p>Initial Value:</p>
    <input type="number"
    ...
    `;
        ...
        const SourceField = `<p>Input Type:</p>
    ...
    const getLabel = (type) => {
        if (!currDomain) {
            return (
                baseNodeTypes.find((n) => n.type === type)?.defaultLabel ?? type
            );
        }
